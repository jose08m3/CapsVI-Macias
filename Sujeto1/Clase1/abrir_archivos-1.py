...
def Normaliza_unidad(array):
    X_normalized = np.array(np.zeros(array.shape, dtype=np.float))
    for i in range(0, array.shape[0]):
        for l in range(0, array.shape[1]):
            minimo = abs(min(array[i][l][:]))
            X_normalized[i][l][:] = (array[i][l][:]+minimo)
            maximo = max(X_normalized[i][l][:])
            X_normalized[i][l][:] = (X_normalized[i][l][:]/maximo)
    return X_normalized
...
#SUJETO 1, AvsRE, de 0-96, AUvsUI 96-197, UIvsRE 198-296
...
# conjunto_WT = np.array(np.zeros((conjunto_entrenar.shape[0], conjunto_entrenar.shape[1], 56)),dtype=np.float)
# for l in range(0,conjunto_entrenar.shape[0]):
#     for i in range(0,conjunto_entrenar.shape[1]):
#         coeffs2 = pywt.wavedec(conjunto_entrenar[l][i][:], 'db4', level = 5)         
#         conjunto_WT[l][i][:] = np.concatenate((coeffs2[3],coeffs2[2],coeffs2[1],coeffs2[0]), axis = 0)

# conjunto_WT = Normaliza_unidad(conjunto_WT)

# conjunto_entrenar = Normaliza_unidad(conjunto_entrenar)
...
caps2_n_caps = 2
clases = caps2_n_caps
# caps2_n_dims = 20 # sujeto1
caps2_n_dims = 20 #sujeto2
...
for train, test in kfold.split(conjunto_entrenar, etiquetas):
    ...
    with tf.Session() as sess:
        ...
        for iteration in range(1, n_iterations_test + 1):
            ...
            loss_test, acc_test, margin_loss_train, caps2_output_norm_eval = sess.run(
                    [loss, accuracy, margin_loss,caps2_output_norm1],
    ...
    # ###############EVALUATION set completo
    # n_iterations_test = int(conjunto_entrenar.shape[0]/batch_size)
    # with tf.Session() as sess:
    #     saver.restore(sess, checkpoint_path)
    ...
    #     for iteration in range(1, n_iterations_test + 1):
    #         limite_low = batch_size*(iteration-1)
    #         X_batch_test = conjunto_entrenar[limite_low:(batch_size*iteration)][:][:]
    #         X_batch_test = np.expand_dims(X_batch_test, axis=3)
    #         y_batch = np.reshape(etiquetas[:], etiquetas.shape[0])
    #         y_batch_test = y_batch[limite_low:(batch_size*iteration)]
    ...
    #     loss_test = np.mean(loss_tests)
    #     acc_test = np.mean(acc_tests)
    #     print("\rFinal test completo accuracy: {:.4f}%  Loss: {:.6f}".format(
    #         acc_test * 100, loss_test))
    ...
    # with tf.Session() as sess:
    #     saver.restore(sess, checkpoint_path)
    #     caps2_output_value, decoder_output_value, y_pred_value, caps2_output_norm_value = sess.run(
    #         [caps2_output, decoder_output, y_pred, caps2_output_norm1],
